...
stopifnot(all.equal(row_oneway_equalvar(x, g), row_oneway_equalvar(X, g)))
...
g <- c("a","a","b","b")
stopifnot(all.equal(nrow(row_oneway_equalvar(x, g)), 0))
...
g2 <- c("a","a","b","b")
stopifnot(all.equal(row_oneway_equalvar(x, g1), row_oneway_equalvar(x, g2)))
...
res1 <- row_oneway_equalvar(x, g)
res2 <- row_oneway_equalvar(x[!is.na(x)], g[!is.na(x)])
...
g <- rep(letters[1:2], each=5)
res1 <- row_oneway_equalvar(x1, g)
res2 <- row_oneway_equalvar(x2, g)
stopifnot(all.equal(res1, res2))

# missing values in g are removed
x <- rnorm(10)
g <- c(NA, NA, rep(letters[1:2], c(3,4)), NA)
res1 <- suppressWarnings(row_oneway_equalvar(x, g))
res2 <- row_oneway_equalvar(x[!is.na(g)], g[!is.na(g)])
stopifnot(all.equal(res1, res2))
...
res1 <- suppressWarnings(row_oneway_equalvar(x, g1))
res2 <- suppressWarnings(row_oneway_equalvar(x, g2))
stopifnot(all.equal(res1, res2))
...
res1 <- suppressWarnings(row_oneway_equalvar(x, g))
res2 <- row_oneway_equalvar(x[!is.na(x) & !is.na(g)], g[!is.na(x) & !is.na(g)])
stopifnot(all.equal(res1, res2))

# everything can be NA
x <- rep(NA_integer_, 4)
g <- rep(NA, 4)
res <- suppressWarnings(row_oneway_equalvar(x, g))
