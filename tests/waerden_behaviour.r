...
stopifnot(all.equal(row_waerden(x, g), row_waerden(X, g)))
...
g <- c("a","a","b","b")
stopifnot(all.equal(nrow(row_waerden(x, g)), 0))
...
g2 <- c("a","a","b","b")
stopifnot(all.equal(row_waerden(x, g1), row_waerden(x, g2)))
...
res1 <- row_waerden(x, g)
res2 <- row_waerden(x[!is.na(x)], g[!is.na(x)])
...
res1 <- suppressWarnings(row_waerden(x, g))
res2 <- row_waerden(x[!is.na(g)], g[!is.na(g)])
stopifnot(all.equal(res1, res2))
...
res1 <- suppressWarnings(row_waerden(x, g1))
res2 <- suppressWarnings(row_waerden(x, g2))
stopifnot(all.equal(res1, res2))
...
res1 <- suppressWarnings(row_waerden(x, g))
res2 <- row_waerden(x[!is.na(x) & !is.na(g)], g[!is.na(x) & !is.na(g)])
stopifnot(all.equal(res1, res2))

# everything can be NA
x <- rep(NA_integer_, 4)
g <- rep(NA, 4)
res <- suppressWarnings(row_waerden(x, g))
