...
stopifnot(all.equal(row_flignerkilleen(x, g), row_flignerkilleen(X, g)))
...
g <- c("a","a","b","b")
stopifnot(all.equal(nrow(row_flignerkilleen(x, g)), 0))
...
g2 <- c("a","a","a","b","b")
stopifnot(all.equal(row_flignerkilleen(x, g1), row_flignerkilleen(x, g2)))
...
res2 <- row_flignerkilleen(x[!is.na(x)], g[!is.na(x)])
...
g <- rep(letters[1:2], each=5)
res1 <- row_flignerkilleen(x1, g)
res2 <- row_flignerkilleen(x2, g)
stopifnot(all.equal(res1, res2))
...
res2 <- row_flignerkilleen(x[!is.na(g)], g[!is.na(g)])
stopifnot(all.equal(res1, res2))
...
res1 <- suppressWarnings(row_flignerkilleen(x, g1))
res2 <- suppressWarnings(row_flignerkilleen(x, g2))
stopifnot(all.equal(res1, res2))
...
res1 <- suppressWarnings(row_flignerkilleen(x, g))
res2 <- row_flignerkilleen(x[!is.na(x) & !is.na(g)], g[!is.na(x) & !is.na(g)])
stopifnot(all.equal(res1, res2))

# everything can be NA
x <- rep(NA_integer_, 4)
g <- rep(NA, 4)
res <- suppressWarnings(row_flignerkilleen(x, g))
