...
X <- matrix(x, nrow=1)
Y <- matrix(y, nrow=1)
stopifnot(all.equal(row_f_var(x, y), row_f_var(X, Y)))
...
stopifnot(all.equal(nrow(row_f_var(x, y)), 0))
...
x <- rnorm(5)
y <- rnorm(5)
stopifnot(all.equal(row_f_var(x, y, alternative="greater"), row_f_var(x, y, alternative="g")))

# conf.level can be 0
x <- rnorm(5)
y <- rnorm(5)
res <- row_f_var(x, y, conf.level=0)
stopifnot(all.equal(res$conf.low, res$conf.high))

# conf.level can be 1
x <- rnorm(5)
y <- rnorm(5)
res <- row_f_var(x, y, conf.level=1)
stopifnot(all.equal(res$conf.low, 0))
...
x <- matrix(rnorm(10), nrow=2)
y <- matrix(rnorm(10), nrow=2)
res1 <- row_f_var(x, y, alternative="g")
res2 <- row_f_var(x, y, alternative=c("g","g"))
stopifnot(all.equal(res1, res2))

# conf.level can be specified for each row
x <- matrix(rnorm(10), nrow=2)
y <- matrix(rnorm(10), nrow=2)
res1 <- row_f_var(x, y, conf.level=0.98)
res2 <- row_f_var(x, y, conf.level=c(0.98,0.98))
stopifnot(all.equal(res1, res2))
...
res1 <- row_f_var(x, y)
res2 <- row_f_var(x[!is.na(x)], y[!is.na(y)])
stopifnot(all.equal(res1, res2))
...
y1 <- c(NA,NA,2:8,NA)
y2 <- c(NaN,NaN,2:8,NaN)
res1 <- row_f_var(x1, y1)
res2 <- row_f_var(x2, y2)
stopifnot(all.equal(res1, res2))

# everything can be NA
x <- rep(NA_integer_, 4)
y <- rep(NA_integer_, 4)
res <- suppressWarnings(row_f_var(x, y))
stopifnot(all.equal(res$obs.x, 0))
stopifnot(all.equal(res$obs.y, 0))
