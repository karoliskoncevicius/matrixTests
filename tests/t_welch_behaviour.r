...
X <- matrix(x, nrow=1)
Y <- matrix(y, nrow=1)
stopifnot(all.equal(row_t_welch(x, y), row_t_welch(X, Y)))
...
stopifnot(all.equal(nrow(row_t_welch(x, y)), 0))
...
x <- rnorm(5)
y <- rnorm(5)
res <- row_t_welch(x, y, conf.level=0)
stopifnot(all.equal(res$conf.low, res$conf.high))

# conf.level can be 1
x <- rnorm(5)
y <- rnorm(5)
res <- row_t_welch(x, y, conf.level=1)
...
x <- rnorm(5)
y <- rnorm(5)
res <- row_t_welch(x, y, conf.level=NA)
...
x <- matrix(rnorm(10), nrow=2)
y <- matrix(rnorm(10), nrow=2)
res1 <- row_t_welch(x, y, alternative="g")
res2 <- row_t_welch(x, y, alternative=c("g","g"))
stopifnot(all.equal(res1, res2))

# conf.level can be specified for each row
x <- matrix(rnorm(10), nrow=2)
y <- matrix(rnorm(10), nrow=2)
res1 <- row_t_welch(x, y, conf.level=0.98)
res2 <- row_t_welch(x, y, conf.level=c(0.98,0.98))
stopifnot(all.equal(res1, res2))
...
res1 <- row_t_welch(x, y)
res2 <- row_t_welch(x[!is.na(x)], y[!is.na(y)])
stopifnot(all.equal(res1, res2))
...
res1 <- row_t_welch(x1, y1)
res2 <- row_t_welch(x2, y2)
stopifnot(all.equal(res1, res2))

# everything can be NA
x <- rep(NA_integer_, 4)
y <- rep(NA_integer_, 4)
res <- suppressWarnings(row_t_welch(x, y))
