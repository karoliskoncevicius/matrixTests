...
stopifnot(all.equal(row_oneway_welch(x, g), row_oneway_welch(X, g)))
...
g <- c("a","a","b","b")
stopifnot(all.equal(nrow(row_oneway_welch(x, g)), 0))
...
g2 <- c("a","a","b","b")
stopifnot(all.equal(row_oneway_welch(x, g1), row_oneway_welch(x, g2)))
...
g <- rep(letters[1:2], each=5)
res1 <- row_oneway_welch(x1, g)
res2 <- row_oneway_welch(x2, g)
stopifnot(all.equal(res1, res2))
...
res1 <- suppressWarnings(row_oneway_welch(x, g))
res2 <- row_oneway_welch(x[!is.na(g)], g[!is.na(g)])
stopifnot(all.equal(res1, res2))
...
res1 <- suppressWarnings(row_oneway_welch(x, g1))
res2 <- suppressWarnings(row_oneway_welch(x, g2))
stopifnot(all.equal(res1, res2))
...
res1 <- suppressWarnings(row_oneway_welch(x, g))
res2 <- row_oneway_welch(x[!is.na(x) & !is.na(g)], g[!is.na(x) & !is.na(g)])
stopifnot(all.equal(res1, res2))

# everything can be NA
x <- rep(NA_integer_, 4)
g <- rep(NA, 4)
res <- suppressWarnings(row_oneway_welch(x, g))
...
g <- letters[1:10]
res1 <- suppressWarnings(row_oneway_welch(x, g))
res2 <- suppressWarnings(row_oneway_welch(NA_integer_, NA))
stopifnot(all.equal(res1, res2))
...
res1 <- suppressWarnings(row_oneway_welch(x, g))
res2 <- suppressWarnings(row_oneway_welch(x[g %in% names(which(table(g)!=1))], g[g %in% names(which(table(g)!=1))]))
stopifnot(all.equal(res1, res2))
...
res1 <- suppressWarnings(row_oneway_welch(x, g))
res2 <- suppressWarnings(row_oneway_welch(x[!is.na(x) & !is.na(g)], g[!is.na(x) & !is.na(g)]))
stopifnot(all.equal(res1, res2))
stopifnot(all.equal(res1$obs.tot, 9))
stopifnot(all.equal(res1$obs.groups, 3))
