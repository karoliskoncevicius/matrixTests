...
X <- matrix(x, nrow=1)
Y <- matrix(y, nrow=1)
stopifnot(all.equal(row_t_paired(x, y), row_t_paired(X, Y)))
...
stopifnot(all.equal(nrow(row_t_paired(x, y)), 0))
...
x <- rnorm(5)
y <- rnorm(5)
stopifnot(all.equal(row_t_paired(x, y, alternative="greater"), row_t_paired(x, y, alternative="g")))

# null can be +Inf
x <- rnorm(5)
y <- rnorm(5)
res <- row_t_paired(x, y, null=Inf)
...
x <- rnorm(5)
y <- rnorm(5)
res <- row_t_paired(x, y, conf.level=0)
stopifnot(all.equal(res$conf.low, res$conf.high))

# conf.level can be 1
x <- rnorm(5)
y <- rnorm(5)
res <- row_t_paired(x, y, conf.level=1)
...
y <- 1:5
res1 <- row_t_paired(x, y)
res2 <- row_t_paired(x, rbind(y,y))
stopifnot(all.equal(res1, res2))

# null can be specified for each row
x <- matrix(rnorm(10), nrow=2)
y <- matrix(rnorm(10), nrow=2)
res1 <- row_t_paired(x, y, null=2)
res2 <- row_t_paired(x, y, null=c(2,2))
stopifnot(all.equal(res1, res2))

# alternative can be specified for each row
x <- matrix(rnorm(10), nrow=2)
y <- matrix(rnorm(10), nrow=2)
res1 <- row_t_paired(x, y, alternative="g")
res2 <- row_t_paired(x, y, alternative=c("g","g"))
stopifnot(all.equal(res1, res2))

# conf.level can be specified for each row
x <- matrix(rnorm(10), nrow=2)
y <- matrix(rnorm(10), nrow=2)
res1 <- row_t_paired(x, y, conf.level=0.98)
res2 <- row_t_paired(x, y, conf.level=c(0.98,0.98))
stopifnot(all.equal(res1, res2))
...
y <- rnorm(10)
res1 <- row_t_paired(x, y)
res2 <- row_t_paired(x[!is.na(x)], y[!is.na(x)])
res2[,c(2,5,8)]  <- res1[,c(2,5,8)]   # NOTE: row_t_paired uses all values to calculate stats (even the ones that do not have a pair)
stopifnot(all.equal(res1, res2))
...
res1 <- row_t_paired(x, y)
res2 <- row_t_paired(x[!is.na(y)], y[!is.na(y)])
res2[,c(1,4,7)]  <- res1[,c(1,4,7)]   # NOTE: row_t_paired uses all values to calculate stats (even the ones that do not have a pair)
stopifnot(all.equal(res1, res2))
...
res1 <- row_t_paired(x, y)
res2 <- row_t_paired(x[!is.na(x) & !is.na(y)], y[!is.na(x) & !is.na(y)])
res2[,c(1,2,4,5,7,8)]  <- res1[,c(1,2,4,5,7,8)]  # NOTE: row_t_paired uses all values to calculate stats (even the ones that do not have a pair)
stopifnot(all.equal(res1, res2))
...
res1 <- row_t_paired(x1, y1)
res2 <- row_t_paired(x2, y2)
stopifnot(all.equal(res1, res2))

# everything can be NA
x <- rep(NA_integer_, 4)
y <- rep(NA_integer_, 4)
res <- suppressWarnings(row_t_paired(x, y))
