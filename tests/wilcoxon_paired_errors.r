...
res <- capture(row_wilcoxon_paired(x=matrix(rnorm(10), nrow=2), y=matrix(rnorm(10), nrow=2), alternative=c("g","c")))
stopifnot(all.equal(res$error, err))
...
res <- capture(row_wilcoxon_paired(x=1:3, y=1:3, mu="1"))
stopifnot(all.equal(res$error, err))

# cannot be complex
res <- capture(row_wilcoxon_paired(x=1:3, y=1:3, mu=complex(1)))
stopifnot(all.equal(res$error, err))

# cannot be in a list
res <- capture(row_wilcoxon_paired(x=1:3, y=1:3, mu=list(1)))
stopifnot(all.equal(res$error, err))

# cannot be a data frame
res <- capture(row_wilcoxon_paired(x=1:3, y=1:3, mu=data.frame(1)))
stopifnot(all.equal(res$error, err))
...
# cannot be NA
res <- capture(row_wilcoxon_paired(x=1:3, y=1:3, mu=NA_integer_))
stopifnot(all.equal(res$error, err))

# cannot be NaN
res <- capture(row_wilcoxon_paired(x=1:3, y=1:3, mu=NaN))
stopifnot(all.equal(res$error, err))

# TODO: check if can be made to work with Inf
# cannot be Inf
res <- capture(row_wilcoxon_paired(x=1:3, y=1:3, mu=Inf))
stopifnot(all.equal(res$error, err))
...
res <- capture(row_wilcoxon_paired(x, y, mu=c(1,2)))
stopifnot(all.equal(res$error, err))
