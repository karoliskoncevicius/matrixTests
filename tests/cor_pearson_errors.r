...
res <- capture(row_cor_pearson(x=matrix(rnorm(10), nrow=2), y=matrix(rnorm(10), nrow=2), alternative=c("g","c")))
stopifnot(all.equal(res$error, err))
...
# cannot be NA
res <- capture(row_cor_pearson(x=1:3, y=2:4, conf.level=NA_integer_))
stopifnot(all.equal(res$error, err))

# cannot be NaN
res <- capture(row_cor_pearson(x=1:3, y=2:4, conf.level=NaN))
stopifnot(all.equal(res$error, err))
