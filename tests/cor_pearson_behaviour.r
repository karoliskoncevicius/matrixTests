...
X <- matrix(x, nrow=1)
Y <- matrix(y, nrow=1)
stopifnot(all.equal(row_cor_pearson(x, y), row_cor_pearson(X, Y)))
...
stopifnot(all.equal(nrow(row_cor_pearson(x, y)), 0))
...
x <- rnorm(5)
y <- rnorm(5)
stopifnot(all.equal(row_cor_pearson(x, y, alternative="greater"), row_cor_pearson(x, y, alternative="g")))

# conf.level can be 0
x <- rnorm(5)
y <- rnorm(5)
res <- row_cor_pearson(x, y, conf.level=0)
stopifnot(all.equal(res$conf.low, res$conf.high))

# conf.level can be 1
x <- rnorm(5)
y <- rnorm(5)
res <- row_cor_pearson(x, y, conf.level=1)
stopifnot(all.equal(res$conf.low, -1))
stopifnot(all.equal(res$conf.high, 1))

# conf.level can be NA
x <- rnorm(5)
y <- rnorm(5)
res <- row_cor_pearson(x, y, conf.level=NA)
...
y <- 1:5
res1 <- row_cor_pearson(x, y)
res2 <- row_cor_pearson(x, rbind(y,y))
stopifnot(all.equal(res1, res2))

# alternative can be specified for each row
x <- matrix(rnorm(10), nrow=2)
y <- matrix(rnorm(10), nrow=2)
res1 <- row_cor_pearson(x, y, alternative="g")
res2 <- row_cor_pearson(x, y, alternative=c("g","g"))
stopifnot(all.equal(res1, res2))

# conf.level can be specified for each row
x <- matrix(rnorm(10), nrow=2)
y <- matrix(rnorm(10), nrow=2)
res1 <- row_cor_pearson(x, y, conf.level=0.98)
res2 <- row_cor_pearson(x, y, conf.level=c(0.98,0.98))
stopifnot(all.equal(res1, res2))
...
y <- rnorm(10)
res1 <- row_cor_pearson(x, y)
res2 <- row_cor_pearson(x[!is.na(x)], y[!is.na(x)])
stopifnot(all.equal(res1, res2))
...
res1 <- row_cor_pearson(x, y)
res2 <- row_cor_pearson(x[!is.na(y)], y[!is.na(y)])
stopifnot(all.equal(res1, res2))
...
res1 <- row_cor_pearson(x, y)
res2 <- row_cor_pearson(x[!is.na(x) & !is.na(y)], y[!is.na(x) & !is.na(y)])
stopifnot(all.equal(res1, res2))
...
res1 <- row_cor_pearson(x1, y1)
res2 <- row_cor_pearson(x2, y2)
stopifnot(all.equal(res1, res2))

# everything can be NA
x <- rep(NA_integer_, 4)
y <- rep(NA_integer_, 4)
res <- suppressWarnings(row_cor_pearson(x, y))
