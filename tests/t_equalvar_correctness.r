...
base_t_equalvar <- function(mat1, mat2, alt="two.sided", mu=0, conf=0.95) {
...
res1 <- base_t_equalvar(x, y, alts, mus, cfs)
res2 <- row_t_equalvar(x, y, alts, mus, cfs)
...
res1 <- base_t_equalvar(x, y, alt)
res2 <- row_t_equalvar(x, y, alt)
stopifnot(all.equal(res1, res2))
...
res1 <- base_t_equalvar(c(1,2,3), c(0,0,0), mu=2)
res2 <- row_t_equalvar(c(1,2,3), c(0,0,0), mu=2)
