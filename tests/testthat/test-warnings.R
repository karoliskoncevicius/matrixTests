...
test_that("rows should have more than 1 available observation", {
  ...
  expect_true(is.na(tst))
  wrn <- '10 of the rows had less than 2 "x" observations\\. First occurrence at row 1'
  expect_warning(tst <- ttest_onegroup(matrix(1:10, ncol=1))$t.statistic, wrn)
  expect_true(all(is.na(tst)))
  mat <- matrix(rnorm(100), ncol=10)
  mat[4:6,1:9] <- NA
  wrn <- '3 of the rows had less than 2 "x" observations\\. First occurrence at row 4'
...
test_that("rows should not be constant", {
  ...
  expect_true(is.na(tst))
  wrn <- '10 of the rows were essentially constant\\. First occurrence at row 1'
  ...
  wrn <- '3 of the rows were essentially constant\\. First occurrence at row 4'
  expect_warning(tst <- ttest_onegroup(mat)$t.statistic, wrn)
  expect_true(all(is.na(tst[4:6])))
})
...
test_that("rows should have at least 3 observations", {
  ...
  expect_true(is.na(tst))
  wrn <- '10 of the rows had less than 3 total observations\\. First occurrence at row 1'
  ...
  matY[4:6,1] <- NA
  matY[2:3,] <- NA
  wrn <- '5 of the rows had less than 3 total observations\\. First occurrence at row 2'
  expect_warning(tst <- ttest_equalvar(matX, matY)$t.statistic, wrn)
  expect_true(all(is.na(tst[2:6])))
...
test_that("one group should have at least one observation", {
  ...
  matY[4:6,1] <- NA
  matY[2:3,] <- NA
  wrnX <- '2 of the rows had zero "x" observations\\. First occurrence at row 2'
  wrnY <- '2 of the rows had zero "y" observations\\. First occurrence at row 2'
  expect_warning(tst <- ttest_equalvar(matY, matX)$t.statistic, wrnX)
  expect_true(all(is.na(tst[2:3])))
...
test_that("rows should have at least 2 observations in each group", {
  ...
  expect_true(is.na(tst))
  wrnX <- '10 of the rows had less than 2 "x" observations\\. First occurrence at row 1'
  wrnY <- '10 of the rows had less than 2 "y" observations\\. First occurrence at row 1'
  ...
  expect_true(all(is.na(tst)))
  wrnX <- '5 of the rows had less than 2 "x" observations\\. First occurrence at row 2'
  wrnY <- '3 of the rows had less than 2 "y" observations\\. First occurrence at row 4'
  matX <- matrix(rnorm(20), ncol=2)
  matY <- matrix(rnorm(20), ncol=2)
  matX[4:6,1] <- NA
  matX[2:3,] <- NA
...
test_that("rows should have at least 2 paired observations", {
  ...
  expect_true(is.na(tst))
  wrn <- '10 of the rows had less than 2 paired observations\\. First occurrence at row 1'
  ...
  matY[4:6,1] <- NA
  matY[2:3,] <- NA
  wrn <- '5 of the rows had less than 2 paired observations\\. First occurrence at row 2'
...
test_that("rows should have at least 2 groups", {
  ...
  expect_true(is.na(fst))
  wrn <- '5 of the rows had less than 2 groups with enough observations\\. First occurrence at row 2'
  mat <- matrix(rnorm(100), ncol=10)
  mat[2,c(1:3,7)] <- NA
  mat[3:4,c(1:6,7)] <- NA
  mat[5:6,c(3:6,7)] <- NA
...
test_that("rows should have more than 1 observation per group", {
  ...
  expect_true(is.na(fst))
  wrn <- '3 of the rows had one observation per group\\. First occurrence at row 2'
  mat <- matrix(rnorm(100), ncol=10)
  mat[2,c(1,2,4,6)] <- NA
  mat[3,c(1,2,3,4,6)] <- NA
  mat[4,c(2,3,4,5,7)] <- NA
  grp <- c(1,1,1,2,2,3,3,NA,NA,NA)
  expect_warning(fst <- oneway_equalvar(mat, grp)$F.statistic, wrn)
  expect_true(all(is.na(fst[2:4])))
})

test_that("all values within one group should not be constant", {
  wrn <- '1 of the rows had essentially perfect fit\\. First occurrence at row 1'
...
test_that("rows should have more than 1 observation in each group", {
  wrn <- '1 of the rows had less than 2 observations per group\\. First occurrence at row 1'
  ...
  expect_true(is.na(fst))
  wrn <- '4 of the rows had less than 2 observations per group\\. First occurrence at row 2'
  mat <- matrix(rnorm(100), ncol=10)
  mat[2,c(1,2,4,6)] <- NA
  mat[3,c(1,2,3,4,6)] <- NA
  mat[4,c(2,3,4,5,7)] <- NA
...
test_that("rows should have at least 2 observations", {
  ...
  expect_true(is.na(kst))
  wrn <- '3 of the rows had less than 2 total observations\\. First occurrence at row 2'
  mat <- matrix(rnorm(100), ncol=10)
  mat[2,c(2:4)] <- NA
  mat[3:4,c(1:6)] <- NA
  grp <- c(1,1,1,2,NA,NA,NA,NA,NA,NA)
  expect_warning(kst <- kruskalwallis(mat, grp)$chsq, wrn)
  expect_true(all(is.na(kst[2:4])))
})
...
test_that("all groups should have at least 2 observations", {
  wrn <- '1 of the rows had groups with less than 2 observations\\. First occurrence at row 1'
  ...
  expect_equal(ngr, 2)
  wrn <- '4 of the rows had groups with less than 2 observations\\. First occurrence at row 2'
  mat <- matrix(rnorm(100), ncol=10)
  ...
  expect_equal(ngr, c(4,3,3,3,2,4,4,4,4,4))
})
...
test_that("all groups should have some variability", {
  ...
  expect_true(is.na(ksq))
  wrn <- '1 of the rows had groups with zero variance\\. First occurrence at row 1'
