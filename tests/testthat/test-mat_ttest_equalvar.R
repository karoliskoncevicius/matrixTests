...
ttest_equalvar <- function(mat1, mat2, alt="two.sided", mu=0, conf=0.95) {
...
test_that("monte-carlo random testing gives equal results", {
  ...
  t1 <- ttest_equalvar(X, Y, alts, mus, cfs)
  t2 <- mat_ttest_equalvar(X, Y, alts, mus, cfs)

  expect_equal(t1, t2)
})

test_that("edge cases give equal results", {
  expect_equal(ttest_equalvar(1:2, 1), mat_ttest_equalvar(1:2, 1))
  expect_equal(ttest_equalvar(1, 1:2), mat_ttest_equalvar(1, 1:2))
  vals1 <- c(1.00000000000001, 1.00000000000002)
  vals2 <- c(1.00000000000001)
  expect_equal(ttest_equalvar(vals1, vals2), mat_ttest_equalvar(vals1, vals2))
})
...
test_that("unsolvable situations produce warnings", {
  ...
  expect_warning(mat_ttest_equalvar(c(1,1,1), c(2,2,2)), er)
  er <- "2 of the rows had less than 3 total observations"
  expect_warning(mat_ttest_equalvar(matrix(1:2, ncol=1), matrix(1:2, ncol=1)), er)
  er <-  "1 of the rows had zero 'x' observations"
  expect_warning(mat_ttest_equalvar(NA_integer_, 1:3), er)
  er <-  "1 of the rows had zero 'y' observations"
  expect_warning(mat_ttest_equalvar(1:3, NA_integer_), er)
})
...
test_that("x cannot be missing", {
  ...
  expect_error(mat_ttest_equalvar(), er)
  expect_error(mat_ttest_equalvar(y=1:2), er)
})

test_that("y cannot be missing", {
  er <- 'argument "y" is missing, with no default'
  expect_error(mat_ttest_equalvar(1:2), er)
})
...
test_that("invalid x values produce errors", {
  ...
  expect_error(mat_ttest_equalvar(iris, mat), er)
  expect_error(mat_ttest_equalvar(matrix(LETTERS[1:10], ncol=2), mat), er)
  expect_error(mat_ttest_equalvar(list(1:10), mat), er)
  expect_error(mat_ttest_equalvar(NA, mat), er)
  expect_error(mat_ttest_equalvar(NULL, mat), er)
  expect_error(mat_ttest_equalvar(matrix(nrow=0, ncol=5), mat), er)
  expect_error(mat_ttest_equalvar(matrix(nrow=5, ncol=0), mat), er)
})

test_that("invalid y values produce errors", {
  mat <- matrix(1:10, ncol=2)
  er  <- "'y' must be a numeric matrix or a vector"
  expect_error(mat_ttest_equalvar(mat, iris), er)
  expect_error(mat_ttest_equalvar(mat, matrix(LETTERS[1:10], ncol=2)), er)
  expect_error(mat_ttest_equalvar(mat, list(1:10)), er)
  expect_error(mat_ttest_equalvar(mat, NA), er)
  expect_error(mat_ttest_equalvar(mat, NULL), er)
  expect_error(mat_ttest_equalvar(mat, matrix(nrow=0, ncol=5)), er)
  expect_error(mat_ttest_equalvar(mat, matrix(nrow=5, ncol=0)), er)
})

test_that("x and y have the same number of rows", {
  mat <- matrix(1:10, nrow=5)
  er <- "'x' and 'y' must have the same number of rows"
  expect_error(mat_ttest_equalvar(mat, mat[1:2,]), er)
})

test_that("data.frame values pass when they are all numeric", {
  expect_error(mat_ttest_equalvar(iris[,-5], iris[,-5]), NA)
})
...
test_that("invalid 'alternative' length", {
  ...
  expect_error(mat_ttest_equalvar(1:2, 1:2, alternative=NULL), er)
  expect_error(mat_ttest_equalvar(1:2, 1:2, alternative=c("t", "g")), er)
})

test_that("invalid 'alternative' values", {
  er <- "all 'alternative' values must be in: two.sided, less, greater"
  expect_error(mat_ttest_equalvar(1:2, 1:2, alternative=NA), er)
  expect_error(mat_ttest_equalvar(1:2, 1:2, alternative="w"), er)
  expect_error(mat_ttest_equalvar(1:2, 1:2, alternative=0), er)
})
...
test_that("invalid 'mu' type or length", {
  ...
  expect_error(mat_ttest_equalvar(1:2, 1:2, mu=NULL), er)
  expect_error(mat_ttest_equalvar(1:2, 1:2, mu=NA), er)
  expect_error(mat_ttest_equalvar(1:2, 1:2, mu="a"), er)
  expect_error(mat_ttest_equalvar(1:2, 1:2, mu=c(NA, 1)), er)
})

test_that("edge cases for mu pass without errors", {
  expect_error(mat_ttest_equalvar(1:2, 1:2, mu=Inf), NA)
  expect_error(mat_ttest_equalvar(1:2, 1:2, mu=-Inf), NA)
  expect_error(mat_ttest_equalvar(1:2, 1:2, mu=NA_integer_), NA)
})
...
test_that("invalid 'conf.level' type or length", {
  ...
  expect_error(mat_ttest_equalvar(1:2, 1:2, conf.level=NULL), er)
  expect_error(mat_ttest_equalvar(1:2, 1:2, conf.level=NA), er)
  expect_error(mat_ttest_equalvar(1:2, 1:2, conf.level="a"), er)
  expect_error(mat_ttest_equalvar(1:2, 1:2, conf.level=c(NA, 1)), er)
})

test_that("invalid 'conf.level' values", {
  er <- "all 'conf.level' values must be between: 0 and 1"
  expect_error(mat_ttest_equalvar(1:2, 1:2, conf.level=NA_integer_), er)
  expect_error(mat_ttest_equalvar(1:2, 1:2, conf.level=1.1), er)
})
