...
base_ttest_paired <- function(mat1, mat2, alt="two.sided", mu=0, conf=0.95) {
  ...
  # for pooled var
  data.frame(mean.x=rowMeans(mat1, na.rm=TRUE),
             mean.y=rowMeans(mat2, na.rm=TRUE),
             mean.diff=unname(sapply(res, "[[", "estimate")),
             var.x=apply(mat1, 1, var, na.rm=TRUE),
             var.y=apply(mat2, 1, var, na.rm=TRUE),
             var.diff=apply(mat1-mat2, 1, var, na.rm=TRUE),
             obs.x=rowSums(!is.na(mat1)),
             obs.y=rowSums(!is.na(mat2)),
             obs.pair=rowSums(!is.na(mat1-mat2)),
...
test_that("unsolvable situations produce warnings", {
  ...
  expect_warning(ttest_paired(c(1,1,1), c(2,2,2)), er)
  er <- "2 of the rows had less than 2 paired observations"
  expect_warning(ttest_paired(matrix(1:2, ncol=1), matrix(1:2, ncol=1)), er)
  er1 <- "1 of the rows had less than 2 paired observations"
  er2 <- "1 of the rows had none 'y' observations"
  expect_warning(ttest_paired(1:2, c(NA_integer_, NA_integer_)), er1)
  expect_warning(ttest_paired(1:2, c(NA_integer_, NA_integer_)), er2)
})
...
test_that("x cannot be missing", {
  ...
  expect_error(ttest_paired(y=1:2), er)
})

test_that("y cannot be missing", {
  er <- 'argument "y" is missing, with no default'
  expect_error(ttest_paired(1:2), er)
})

test_that("invalid x values produce errors", {
  mat <- matrix(1:10, ncol=2)
  er  <- "'x' must be a numeric matrix or a vector"
  expect_error(ttest_paired(iris, mat), er)
  expect_error(ttest_paired(matrix(LETTERS[1:10], ncol=2), mat), er)
  expect_error(ttest_paired(list(1:10), mat), er)
  expect_error(ttest_paired(NA, mat), er)
  expect_error(ttest_paired(NULL, mat), er)
  expect_error(ttest_paired(matrix(nrow=0, ncol=5), mat), er)
  expect_error(ttest_paired(matrix(nrow=5, ncol=0), mat), er)
})

test_that("invalid y values produce errors", {
  mat <- matrix(1:10, ncol=2)
  er  <- "'y' must be a numeric matrix or a vector"
  expect_error(ttest_paired(mat, iris), er)
  expect_error(ttest_paired(mat, matrix(LETTERS[1:10], ncol=2)), er)
  expect_error(ttest_paired(mat, list(1:10)), er)
  expect_error(ttest_paired(mat, NA), er)
  expect_error(ttest_paired(mat, NULL), er)
  expect_error(ttest_paired(mat, matrix(nrow=0, ncol=5)), er)
  expect_error(ttest_paired(mat, matrix(nrow=5, ncol=0)), er)
})

test_that("x and y have the same number of rows and columns", {
  mat <- matrix(1:20, nrow=5)
  er <- "'x' and 'y' must have the same number of rows"
  expect_error(ttest_paired(mat, mat[1:2,]), er)
  er <- "'x' and 'y' must have the same number of columns"
  expect_error(ttest_paired(mat, mat[,1:2]), er)
})

test_that("data.frame values pass when they are all numeric", {
  expect_error(ttest_paired(iris[,-5], rev(iris[,-5])), NA)
})
...
test_that("invalid 'alternative' length", {
  ...
  expect_error(ttest_paired(1:2, 1:2, alternative=NULL), er)
  expect_error(ttest_paired(1:2, 1:2, alternative=c("t", "g")), er)
})

test_that("invalid 'alternative' values", {
  er <- "all 'alternative' values must be in: two.sided, less, greater"
  expect_error(ttest_paired(1:2, 1:2, alternative=NA), er)
  expect_error(ttest_paired(1:2, 1:2, alternative="w"), er)
  expect_error(ttest_paired(1:2, 1:2, alternative=0), er)
})
...
test_that("invalid 'mu' type or length", {
  ...
  expect_error(ttest_paired(1:2, 1:2, mu=NULL), er)
  expect_error(ttest_paired(1:2, 1:2, mu=NA), er)
  expect_error(ttest_paired(1:2, 1:2, mu="a"), er)
  expect_error(ttest_paired(1:2, 1:2, mu=c(NA, 1)), er)
})

test_that("edge cases for mu pass without errors", {
  expect_error(ttest_paired(1:2, 2:1, mu=Inf), NA)
  expect_error(ttest_paired(1:2, 2:1, mu=-Inf), NA)
  expect_error(ttest_paired(1:2, 2:1, mu=NA_integer_), NA)
})
...
test_that("invalid 'conf.level' type or length", {
  ...
  expect_error(ttest_paired(1:2, 1:2, conf.level=NULL), er)
  expect_error(ttest_paired(1:2, 1:2, conf.level=NA), er)
  expect_error(ttest_paired(1:2, 1:2, conf.level="a"), er)
  expect_error(ttest_paired(1:2, 1:2, conf.level=c(NA, 1)), er)
})

test_that("invalid 'conf.level' values", {
  er <- "all 'conf.level' values must be between: 0 and 1"
  expect_error(ttest_paired(1:2, 1:2, conf.level=NA_integer_), er)
  expect_error(ttest_paired(1:2, 1:2, conf.level=1.1), er)
})
