...
base_ttest_welch <- function(mat1, mat2, alt="two.sided", mu=0, conf=0.95) {
  ...
  # for pooled var
  data.frame(mean.x=unname(sapply(res, "[[", "estimate")[1,]),
             mean.y=unname(sapply(res, "[[", "estimate")[2,]),
             mean.diff=-unname(sapply(lapply(res, getElement, "estimate"), diff)),
             var.x=apply(mat1, 1, var, na.rm=TRUE),
             var.y=apply(mat2, 1, var, na.rm=TRUE),
             obs.x=rowSums(!is.na(mat1)),
             obs.y=rowSums(!is.na(mat2)),
             obs.tot=rowSums(!is.na(mat1))+rowSums(!is.na(mat2)),
...
test_that("unsolvable situations produce warnings", {
  ...
  expect_warning(ttest_welch(c(1,1,1), c(2,2,2)), er)
  er <- "2 of the rows had less than 2 'x' observations"
  expect_warning(ttest_welch(matrix(1:2, ncol=1), matrix(1:10, ncol=5)), er)
  er <- "1 of the rows had less than 2 'y' observations"
  expect_warning(ttest_welch(1:3, NA_integer_), er)
})
...
test_that("x cannot be missing", {
  ...
  expect_error(ttest_welch(y=1:2), er)
})

test_that("y cannot be missing", {
  er <- 'argument "y" is missing, with no default'
  expect_error(ttest_welch(1:2), er)
})

test_that("invalid x values produce errors", {
  mat <- matrix(1:10, ncol=2)
  er  <- "'x' must be a numeric matrix or a vector"
  expect_error(ttest_welch(iris, mat), er)
  expect_error(ttest_welch(matrix(LETTERS[1:10], ncol=2), mat), er)
  expect_error(ttest_welch(list(1:10), mat), er)
  expect_error(ttest_welch(NA, mat), er)
  expect_error(ttest_welch(NULL, mat), er)
  expect_error(ttest_welch(matrix(nrow=0, ncol=5), mat), er)
  expect_error(ttest_welch(matrix(nrow=5, ncol=0), mat), er)
})

test_that("invalid y values produce errors", {
  mat <- matrix(1:10, ncol=2)
  er  <- "'y' must be a numeric matrix or a vector"
  expect_error(ttest_welch(mat, iris), er)
  expect_error(ttest_welch(mat, matrix(LETTERS[1:10], ncol=2)), er)
  expect_error(ttest_welch(mat, list(1:10)), er)
  expect_error(ttest_welch(mat, NA), er)
  expect_error(ttest_welch(mat, NULL), er)
  expect_error(ttest_welch(mat, matrix(nrow=0, ncol=5)), er)
  expect_error(ttest_welch(mat, matrix(nrow=5, ncol=0)), er)
})

test_that("x and y have the same number of rows", {
  mat <- matrix(1:10, nrow=5)
  er <- "'x' and 'y' must have the same number of rows"
  expect_error(ttest_welch(mat, mat[1:2,]), er)
})

test_that("data.frame values pass when they are all numeric", {
  expect_error(ttest_welch(iris[,-5], iris[,-5]), NA)
})
...
test_that("invalid 'alternative' length", {
  ...
  expect_error(ttest_welch(1:2, 1:2, alternative=NULL), er)
  expect_error(ttest_welch(1:2, 1:2, alternative=c("t", "g")), er)
})

test_that("invalid 'alternative' values", {
  er <- "all 'alternative' values must be in: two.sided, less, greater"
  expect_error(ttest_welch(1:2, 1:2, alternative=NA), er)
  expect_error(ttest_welch(1:2, 1:2, alternative="w"), er)
  expect_error(ttest_welch(1:2, 1:2, alternative=0), er)
})
...
test_that("invalid 'mu' type or length", {
  ...
  expect_error(ttest_welch(1:2, 1:2, mu=NULL), er)
  expect_error(ttest_welch(1:2, 1:2, mu=NA), er)
  expect_error(ttest_welch(1:2, 1:2, mu="a"), er)
  expect_error(ttest_welch(1:2, 1:2, mu=c(NA, 1)), er)
})

test_that("edge cases for mu pass without errors", {
  expect_error(ttest_welch(1:2, 1:2, mu=Inf), NA)
  expect_error(ttest_welch(1:2, 1:2, mu=-Inf), NA)
  expect_error(ttest_welch(1:2, 1:2, mu=NA_integer_), NA)
})
...
test_that("invalid 'conf.level' type or length", {
  ...
  expect_error(ttest_welch(1:2, 1:2, conf.level=NULL), er)
  expect_error(ttest_welch(1:2, 1:2, conf.level=NA), er)
  expect_error(ttest_welch(1:2, 1:2, conf.level="a"), er)
  expect_error(ttest_welch(1:2, 1:2, conf.level=c(NA, 1)), er)
})

test_that("invalid 'conf.level' values", {
  er <- "all 'conf.level' values must be between: 0 and 1"
  expect_error(ttest_welch(1:2, 1:2, conf.level=NA_integer_), er)
  expect_error(ttest_welch(1:2, 1:2, conf.level=1.1), er)
})
