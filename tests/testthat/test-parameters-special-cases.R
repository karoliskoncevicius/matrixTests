...
test_that("MU can be Infinite", {
  ...
  y <- t(iris[76:150,-5])
  # Inf
  expect_equal(row.t.onesample(x=x, mu=Inf)$statistic, rep(-Inf, nrow(x)))
  expect_equal(row.t.equalvar(x=x, y=y, mu=Inf)$statistic, rep(-Inf, nrow(x)))
  expect_equal(row.t.welch(x=x, y=y, mu=Inf)$statistic, rep(-Inf, nrow(x)))
  expect_equal(row.t.paired(x=x, y=y, mu=Inf)$statistic, rep(-Inf, nrow(x)))
  # -Inf
  expect_equal(row.t.onesample(x=x, mu=-Inf)$statistic, rep(Inf, nrow(x)))
  expect_equal(row.t.equalvar(x=x, y=y, mu=-Inf)$statistic, rep(Inf, nrow(x)))
  expect_equal(row.t.welch(x=x, y=y, mu=-Inf)$statistic, rep(Inf, nrow(x)))
