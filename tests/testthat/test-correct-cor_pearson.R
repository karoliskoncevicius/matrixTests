...
test_that("perfect correlations give equal results", {
  ...
  t2 <- suppressWarnings(cor_pearson(1:4, 1:4))
  expect_equal(t1, t2)

  # negative case
  t1 <- base_cor_pearson(1:4, 4:1)
  t2 <- suppressWarnings(cor_pearson(1:4, 4:1))
  expect_equal(t1, t2)
})
...
test_that("minumum allowed sample sizes give equal results", {
  ...
  t2 <- suppressWarnings(cor_pearson(x, y))
  expect_equal(t1, t2)
...
test_that("parameter edge cases give equal results", {
  ...
  t2 <- suppressWarnings(cor_pearson(X1, X2, pars[,1], pars[,2]))
...
test_that("warning when rows have exactly 3 complete observations", {
  ...
  # standard case
  expect_warning(res <- cor_pearson(c(1,1,2), c(1,2,3)), wrn, all=TRUE)
  expect_true(all(is.na(res[,colnames(res) %in% nacolumns])))
  expect_true(all(!is.na(res[,!colnames(res) %in% nacolumns])))

  # with NAs present
  expect_warning(res <- cor_pearson(c(1,1,4,NA,4), c(1,2,1,3,NA)), wrn, all=TRUE)
  expect_true(all(is.na(res[,colnames(res) %in% nacolumns])))
  expect_true(all(!is.na(res[,!colnames(res) %in% nacolumns])))
})
