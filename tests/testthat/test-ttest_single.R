context("ttest_single")

################################################################################
############################### HELPER FUNCTIONS ###############################
################################################################################

base_ttest_single <- function(mat, alt="two.sided", mu=0, conf=0.95) {
...
test_that("monte-carlo random testing gives equal results", {
  ...
  t1 <- base_ttest_single(X, alts, mus, cfs)
  t2 <- ttest_single(X, alts, mus, cfs)

  expect_equal(t1, t2)
})

test_that("edge cases give equal results", {
  expect_equal(base_ttest_single(1:2), ttest_single(1:2))
  vals <- c(1.00000000000001, 1.00000000000002)
  expect_equal(base_ttest_single(vals), ttest_single(vals))
  expect_equal(base_ttest_single(1:10^5), ttest_single(1:10^5))
})
...
test_that("unsolvable situations produce warnings", {
  ...
  expect_warning(ttest_single(NA_integer_), er)
  er <- "2 of the rows had less than 2 'x' observations"
  expect_warning(ttest_single(matrix(1:2, ncol=1)), er)
  er <- "1 of the rows were essentially constant"
  expect_warning(ttest_single(c(1,1,1,1,1,1)), er)
})
...
test_that("x cannot be missing", {
  ...
  expect_error(ttest_single(), er)
})

test_that("invalid x values produce errors", {
  er <- "'x' must be a numeric matrix or a vector"
  expect_error(ttest_single(iris), er)
  expect_error(ttest_single(matrix(LETTERS[1:10], ncol=2)), er)
  expect_error(ttest_single(list(1:10)), er)
  expect_error(ttest_single(NA), er)
  expect_error(ttest_single(NULL), er)
  expect_error(ttest_single(matrix(nrow=0, ncol=5)), er)
  expect_error(ttest_single(matrix(nrow=5, ncol=0)), er)
})

test_that("data.frame values pass when they are all numeric", {
  expect_error(ttest_single(iris[,-5]), NA)
})
...
test_that("invalid 'alternative' length", {
  ...
  expect_error(ttest_single(1:2, alternative=NULL), er)
  expect_error(ttest_single(1:2, alternative=c("t", "g")), er)
})

test_that("invalid 'alternative' values", {
  er <- "all 'alternative' values must be in: two.sided, less, greater"
  expect_error(ttest_single(1:2, alternative=NA), er)
  expect_error(ttest_single(1:2, alternative="w"), er)
  expect_error(ttest_single(1:2, alternative=0), er)
})
...
test_that("invalid 'mu' type or length", {
  ...
  expect_error(ttest_single(1:2, mu=NULL), er)
  expect_error(ttest_single(1:2, mu=NA), er)
  expect_error(ttest_single(1:2, mu="a"), er)
  expect_error(ttest_single(1:2, mu=c(NA, 1)), er)
})

test_that("edge cases for mu pass without errors", {
  expect_error(ttest_single(1:2, mu=Inf), NA)
  expect_error(ttest_single(1:2, mu=-Inf), NA)
  expect_error(ttest_single(1:2, mu=NA_integer_), NA)
})
...
test_that("invalid 'conf.level' type or length", {
  ...
  expect_error(ttest_single(1:2, conf.level=NULL), er)
  expect_error(ttest_single(1:2, conf.level=NA), er)
  expect_error(ttest_single(1:2, conf.level="a"), er)
  expect_error(ttest_single(1:2, conf.level=c(NA, 1)), er)
})

test_that("invalid 'conf.level' values", {
  er <- "all 'conf.level' values must be between: 0 and 1"
  expect_error(ttest_single(1:2, conf.level=NA_integer_), er)
  expect_error(ttest_single(1:2, conf.level=1.1), er)
})
