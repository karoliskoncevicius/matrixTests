...
test_that("monte-carlo random testing gives equal results", {
  ...
  t2 <- row_t_paired(X, Y, alts, mus, cfs)
...
test_that("parameter edge cases give equal results", {
  ...
  t2 <- row_t_paired(X1, X2, pars[,1], pars[,2], pars[,3])
  expect_equal(t1, t2)
})
...
test_that("warning when a row has less than 2 paired observations", {
  ...
  expect_warning(res <- row_t_paired(NA_integer_, NA_integer_), wrn, all=TRUE)
  ...
  expect_warning(res <- row_t_paired(rep(NA_integer_,3), c(1,2,3)), wrn, all=TRUE)
  expect_true(all(is.na(res[,colnames(res) %in% nacolumns])))
  expect_equal(res$obs.pair, 0)
  expect_equal(res$obs.x, 0)
  ...
  expect_warning(res <- row_t_paired(c(1,2,NA), c(2,NA,3)), wrn, all=TRUE)
  expect_true(all(is.na(res[,colnames(res) %in% nacolumns])))
...
test_that("warning when row has constant values", {
  ...
  expect_warning(res <- row_t_paired(c(1,1,1), c(1,1,1)), wrn, all=TRUE)
  expect_true(all(is.na(res[,colnames(res) %in% nacolumns])))

  # values equal in their group
  expect_warning(res <- row_t_paired(c(1,1), c(2,2)), wrn, all=TRUE)
  expect_true(all(is.na(res[,colnames(res) %in% nacolumns])))

  # equal values in each group + some NAs
  expect_warning(res <- row_t_paired(c(1,1,1,NA), c(NA,2,2,2)), wrn, all=TRUE)
  expect_true(all(is.na(res[,colnames(res) %in% nacolumns])))

  # differences are constant
  expect_warning(res <- row_t_paired(c(3,2,0), c(2,1,-1)), wrn, all=TRUE)
  expect_true(all(is.na(res[,colnames(res) %in% nacolumns])))
})
